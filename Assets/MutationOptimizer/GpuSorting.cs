...
public class GpuSorting
{
	...
	public void Dispatch(Args args)
	{
		...
		m_CS.SetInt("numKeys", (int)constants.numKeys);
		m_CS.SetInt("numBlocksPerThreadGroup", (int)constants.numBlocksPerThreadGroup);
		m_CS.SetInt("numThreadGroups", (int)constants.numThreadGroups);
		m_CS.SetInt("numThreadGroupsWithAdditionalBlocks", (int)constants.numThreadGroupsWithAdditionalBlocks);
		m_CS.SetInt("numReduceThreadgroupPerBin", (int)constants.numReduceThreadgroupPerBin);
		m_CS.SetInt("numScanValues", (int)constants.numScanValues);
		...
		{
			m_CS.SetInt("shift", (int)constants.shift);

			// Sum
			m_CS.SetBuffer(m_KernelSum, "rw_source_keys", srcKeyBuffer);
			m_CS.SetBuffer(m_KernelSum, "rw_sum_table", args.resources.scratchBuffer);
			m_CS.Dispatch(m_KernelSum, (int)numThreadGroupsToRun, 1, 1);

			// Reduce
			m_CS.SetBuffer(m_KernelReduce, "rw_sum_table", args.resources.scratchBuffer);
			m_CS.SetBuffer(m_KernelReduce, "rw_reduce_table", args.resources.reducedScratchBuffer);
			m_CS.Dispatch(m_KernelReduce, (int)numReducedThreadGroupsToRun, 1, 1);

			// Scan
			m_CS.SetBuffer(m_KernelScan, "rw_scan_source", args.resources.reducedScratchBuffer);
			m_CS.SetBuffer(m_KernelScan, "rw_scan_dest", args.resources.reducedScratchBuffer);
			m_CS.Dispatch(m_KernelScan, 1, 1, 1);

			// Scan add
			m_CS.SetBuffer(m_KernelScanAdd, "rw_scan_source", args.resources.scratchBuffer);
			m_CS.SetBuffer(m_KernelScanAdd, "rw_scan_dest", args.resources.scratchBuffer);
			m_CS.SetBuffer(m_KernelScanAdd, "rw_scan_scratch", args.resources.reducedScratchBuffer);
			m_CS.Dispatch(m_KernelScanAdd, (int)numReducedThreadGroupsToRun, 1, 1);

			// Scatter
			m_CS.SetBuffer(m_KernelScatter, "rw_source_keys", srcKeyBuffer);
			m_CS.SetBuffer(m_KernelScatter, "rw_dest_keys", dstKeyBuffer);
			m_CS.SetBuffer(m_KernelScatter, "rw_sum_table", args.resources.scratchBuffer);
			m_CS.SetBuffer(m_KernelScatter, "rw_source_payloads", srcPayloadBuffer);
			m_CS.SetBuffer(m_KernelScatter, "rw_dest_payloads", dstPayloadBuffer);
			m_CS.Dispatch(m_KernelScatter, (int)numThreadGroupsToRun, 1, 1);
